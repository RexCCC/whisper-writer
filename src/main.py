...
def start_app(preloaded_local_model=None):
    ...
    class WhisperWriterApp(QObject):
        def __init__(self, preloaded_local_model=None):
            """
            Initialize the application, opening settings window if no configuration file is found.
            """
            super().__init__()
            ConfigManager.initialize()
            self.preloaded_local_model = preloaded_local_model
        ...
        def initialize_components(self):
            ...
            elif self.preloaded_local_model is not None:
                self.local_model = self.preloaded_local_model
            else:
            ...
            if not model_options.get('use_api') and self.local_model is None:
                self.tray_icon.showMessage(
                    'WhisperWriter',
                    'Speech model failed to load. Check internet for first download, '
                    'or run scripts\\download-model.py from the install folder.',
        ...
        def restart_app(self):
            ...
            args = list(sys.argv) if sys.argv else ["run.py"]
            QProcess.startDetached(sys.executable, args, app_root)
    ...
    app = WhisperWriterApp(preloaded_local_model=preloaded_local_model)
    app.run()
